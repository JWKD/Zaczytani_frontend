...
- **Motyw (`@mui/material`)**:  
  Aplikacja wykorzystuje motyw Material-UI, który jest dostosowywany w pliku `theme.ts`. Motyw jest dostarczany globalnie za pomocą komponentu `ThemeProvider`.

- **Globalny reset stylów (`CssBaseline`)**:  
  Komponent `CssBaseline` zapewnia resetowanie domyślnych stylów CSS przeglądarki, ujednolicając wygląd aplikacji.
